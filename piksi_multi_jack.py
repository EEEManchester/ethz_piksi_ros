...
class PiksiMulti:
    ...
    def __init__(self):
        ...
        #Jack's velocity variables
        self.rtk_vel_north=0
        self.rtk_vel_east=0
        self.rtk_vel_down=0
    ...
    def create_topic_callbacks(self):
        ...
        self.handler.add_callback(self.cb_sbp_odom_fix, msg_type=SBP_MSG_BASELINE_NED) #baseline_ned_callback
        self.handler.add_callback(self.cb_sbp_vel_ned_odom, msg_type=SBP_MSG_VEL_NED_COV) #vel_ned_callback
    ...
    def advertise_topics(self):
        ...
        if self.publish_covariances:
            ...
            publishers['odom_pub'] = rospy.Publisher(rospy.get_name() + '/odom_pub', Odometry, queue_size=10)
    ...
    def cb_sbp_vel_ned_odom(self, msg_raw, **metadata):
        
        msg = MsgVelNEDCov(msg_raw) 
        if msg.flags == VelNedCov.VEL_MODE_INVALID:
            return
        #set "global" velocity variable
        self.rtk_vel_north = msg.n * PiksiMulti.kFromMilli #converts raw message in mm to m
        self.rtk_vel_east = msg.e * PiksiMulti.kFromMilli
        self.rtk_vel_down = msg.d * PiksiMulti.kFromMilli

        #Set Velocity Covariance


    def cb_sbp_odom_fix(self, msg_raw, **metadata):
        ...
        odom.header.frame_id = self.base_ned_frame 

        # Get position in m.
        x_pos = msg.n * self.kFromMilli
        y_pos = msg.e * self.kFromMilli
        z_pos = msg.d * self.kFromMilli

        # Get position covariance
        cov_h_h = (msg.h_accuracy * self.kFromMilli)**2
        cov_v_v = (msg.v_accuracy * self.kFromMilli)**2 

        #Pose x/y/z linear covariance
        odom.pose.covariance[0] = cov_h_h # x = 0, 0 in the 6x6 cov matrix
        odom.pose.covariance[7] = cov_h_h # y = 1, 1 
        odom.pose.covariance[14] = cov_v_v # y = 2, 2
        
        #Pose x/y/z angular covariance. **This is unknown so has been set to 1000**
        # Unsure what this should be set at, may be best to test.
        odom.pose.covariance[21] = 1.0e3  # x rotation = 3, 3
        odom.pose.covariance[28] = 1.0e3 # y rotation = 4,4
        odom.pose.covariance[35] = 1.0e3 # z rotation = 5,5

        # Get velocity in m
        x_vel = self.rtk_vel_east
        y_vel = self.rtk_vel_north
        z_vel = self.rtk_vel_down
